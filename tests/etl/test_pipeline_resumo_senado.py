...
from etl.pipeline_resumo_senado import converter_data_para_timestamp_sp, executar_pipeline_resumo_senado
...
def _make_supabase_mock(proposicoes: list):
    ...
    chain = tabela.select.return_value.not_.is_.return_value.is_.return_value.is_.return_value
...
async def test_pipeline_senado_caminho_feliz_qdrant_first(mock_sleep, mock_pdfplumber_open):
    """
    ...
    assert kwargs_qdrant["points"][0]["payload"]["data_votacao"] == converter_data_para_timestamp_sp("2023-05-10")
    assert kwargs_qdrant["points"][0]["payload"]["proposicao_id_string"] == "pl_123_2023"
...
async def test_pipeline_senado_erro_permanente_extracao(mock_pdfplumber_open):
    ...
    total = await executar_pipeline_resumo_senado(supabase_mock, qdrant_mock, motor_mock, gemini_mock)
    ...
    tabela_mock.update.assert_called_once()
    assert tabela_mock.update.call_args[0][0]["erro_resumo"] == "PDF sem texto extraível ou corrompido"


@pytest.mark.asyncio
@respx.mock
@patch("etl.extrator_texto_senado.pdfplumber.open")
async def test_pipeline_senado_erro_transitorio_nao_salva_no_banco(mock_pdfplumber_open):
    """

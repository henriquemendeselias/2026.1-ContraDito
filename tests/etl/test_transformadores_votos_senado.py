...
def test_gerar_id_voto_senado_deterministico():
    ...
    assert uuid.UUID(id_1).version == 5, "O hash deve ser obrigatoriamente UUID versão 5."
...
def test_encontrar_sessao_merito_senado():
    ...
    """
    sessoes_mock = [
        {"codigoSessao": 1, "dataSessao": "2022-12-01", "descricaoVotacao": "Aprovação do texto-base"}, # Rejeitado: Corte temporal
        {"codigoSessao": 2, "dataSessao": "2023-02-01", "descricaoVotacao": "Votação do Requerimento"},  # Rejeitado: Blocklist
        {"codigoSessao": 3, "dataSessao": "2023-03-01", "descricaoVotacao": "Aprovação do texto-base"},  # APROVADO: É a mais antiga válida
        {"codigoSessao": 4, "dataSessao": "2023-01-15", "descricaoVotacao": "Votação da emenda"},        # Rejeitado: Não tem Allowlist
        {"codigoSessao": 5, "dataSessao": "2023-04-01", "descricaoVotacao": "Votação do substitutivo"},  # Válido, mas mais recente que a 3
    ]
...
def test_encontrar_sessao_merito_senado_retorna_none_se_vazio():
    ...
    assert encontrar_sessao_merito_senado([{"codigoSessao": 1, "dataSessao": "2023-02-01", "descricaoVotacao": "Requerimento"}]) is None
...
def test_filtrar_votos_validos_senado_soft_drop():
    ...
    votos_brutos = [
        ...
        {"codigoParlamentar": 99, "nomeParlamentar": "Suplente Fantasma", "voto": "Não"},

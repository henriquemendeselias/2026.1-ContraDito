...
from etl.extrator_votos_senado import obter_sessao_votacao_senado, extrair_votos_proposicao_senado, executar_pipeline_votos_senado
...
async def test_obter_sessao_votacao_senado_encontra_merito():
    ...
    mock_json = [
        {"codigoSessao": 1, "dataSessao": "2023-01-10", "descricaoVotacao": "Votação do Requerimento"}, # Bloqueado (Blocklist)
        {"codigoSessao": 2, "dataSessao": "2023-02-15", "descricaoVotacao": "Aprovação do texto-base"}, # ALVO (Válido)
        {"codigoSessao": 3, "dataSessao": "2022-12-01", "descricaoVotacao": "Aprovação do substitutivo"} # Bloqueado (Corte Temporal)
    ]
...
async def test_extrair_votos_proposicao_senado_sucesso():
    ...
    mock_json = [
        {
            ...
            "votos": [
                {"codigoParlamentar": 10, "siglaPartidoParlamentar": "PT", "siglaVotoParlamentar": "Sim"},
                {"codigoParlamentar": 99, "siglaPartidoParlamentar": "PSDB", "siglaVotoParlamentar": "Não"}, # Fantasma (Soft Drop)
                {"codigoParlamentar": 20, "siglaPartidoParlamentar": "PL", "siglaVotoParlamentar": "Abstenção"}
            ]
        }
    ]
    respx.get(url).respond(status_code=200, json=mock_json)
    ...
    async with httpx.AsyncClient() as client:
        votos, id_votacao, data_votacao = await extrair_votos_proposicao_senado(client, proposicao_id, id_senado, ids_validos)
...
async def test_extrair_votos_proposicao_senado_resiliencia_retry(mock_sleep):
    ...
    async with httpx.AsyncClient() as client:
        ...
        assert route.call_count == 3, "Deveria ter tentado 3 vezes antes de retornar com sucesso."
        assert id_votacao == 2
        assert len(votos) == 1
...
async def test_executar_pipeline_senado_poison_pill(mock_sleep):
    ...
    mock_supabase.table.return_value.select.return_value.execute.return_value = mock_resp_politicos
    mock_supabase.table.return_value.select.return_value.is_.return_value.execute.return_value = mock_resp_proposicoes

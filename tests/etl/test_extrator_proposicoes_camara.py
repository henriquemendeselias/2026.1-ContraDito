...
async def test_extrair_tramitacoes_proposicao_sucesso():
    ...
    mock_json = {
        "dados": [
            {"codTipoTramitacao": 231, "dataHora": "2023-01-01T10:00:00"}
        ]
    }
    ...
    async with httpx.AsyncClient() as client:
        ...
        tramitacoes = await extrair_tramitacoes_proposicao(client, id_proposicao, semaphore)
...
async def test_extrair_detalhes_proposicao_sucesso():
    ...
    mock_json = {"dados": {"id": id_proposicao, "urlInteiroTeor": "http://camara.gov.br/pec45.pdf"}}
    respx.get(url).respond(status_code=200, json=mock_json)
...
async def test_processar_pagina_proposicoes_orquestracao():
    ...
    respx.get(url_pagina).respond(
        status_code=200,
        json={"dados": [
            {"id": 1, "siglaTipo": "PL", "numero": 10, "ano": 2023}, # Payload simples sem url (como vem na API real)
            {"id": 2, "siglaTipo": "PL", "numero": 11, "ano": 2022}
        ], "links": []}
    )

    # Mock das tramitações para a Proposição 1 (Válida - 2023)
    respx.get("https://dadosabertos.camara.leg.br/api/v2/proposicoes/1/tramitacoes").respond(
        status_code=200,
        json={"dados": [{"codTipoTramitacao": 231, "dataHora": "2023-05-15T10:00:00", "id": "t1"}]}
    )

    # Mock das tramitações para a Proposição 2 (Inválida - 2022 - será descartada)
    respx.get("https://dadosabertos.camara.leg.br/api/v2/proposicoes/2/tramitacoes").respond(
        status_code=200,
        json={"dados": [{"codTipoTramitacao": 231, "dataHora": "2022-12-31T10:00:00", "id": "t2"}]}
    )
    ...
    respx.get("https://dadosabertos.camara.leg.br/api/v2/proposicoes/1").respond(
        status_code=200,
        json={"dados": {"id": 1, "siglaTipo": "PL", "numero": 10, "ano": 2023, "ementa": "Ementa 1", "urlInteiroTeor": "url1"}}
    )

    async with httpx.AsyncClient() as client:
        semaphore = asyncio.Semaphore(2)
        resultados, next_url = await processar_pagina_proposicoes(client, url_pagina, semaphore)
...
async def test_executar_pipeline_completo_integracao(mock_sleep):
    ...
    mock_json_pagina = {
        "dados": [
            {"id": 1, "siglaTipo": "PL", "numero": 10, "ano": 2023, "ementa": "Teste", "urlInteiroTeor": "url1"}
        ],
        "links": []
    }
    respx.get(url_pag_1).respond(status_code=200, json=mock_json_pagina)
    
    url_tramitacao = "https://dadosabertos.camara.leg.br/api/v2/proposicoes/1/tramitacoes"
    ...
    respx.get("https://dadosabertos.camara.leg.br/api/v2/proposicoes/1").respond(
        status_code=200,
        json={"dados": {"id": 1, "siglaTipo": "PL", "numero": 10, "ano": 2023, "ementa": "Teste", "urlInteiroTeor": "url1"}}
    )

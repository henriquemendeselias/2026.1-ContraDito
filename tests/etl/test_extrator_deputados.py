...
from etl.extrator_deputados import extrair_pagina_deputados, extrair_detalhes_deputado, executar_extracao_pagina, executar_pipeline_completo


@respx.mock
def test_extrair_pagina_deputados_sucesso():
    """
    ...
    mock_json = {
        ...
        "links": [
            {"rel": "self", "href": "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=1"},
            {"rel": "next", "href": "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=2"}
        ]
    }
    ...
    assert next_url == "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=2"


@respx.mock
def test_extrair_pagina_deputados_ultima_pagina():
    """
    ...
    """
    url = "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=5"
...
@respx.mock
@patch("time.sleep")
def test_extrair_pagina_deputados_retry_sucesso(mock_sleep):
    """
...
@respx.mock
@patch("time.sleep")
def test_extrair_pagina_deputados_falha_total(mock_sleep):
    """
...
@respx.mock
@patch("time.sleep")
def test_extrair_detalhes_deputado_sucesso(mock_sleep):
    """
...
@respx.mock
@patch("time.sleep")
def test_extrair_detalhes_deputado_retry_sucesso(mock_sleep):
    """
...
@respx.mock
@patch("time.sleep")
def test_extrair_detalhes_deputado_falha_total(mock_sleep):
    """
...
@respx.mock
@patch("time.sleep")
def test_executar_extracao_pagina_com_upsert(mock_sleep):
    """
    ...
    """
    url_pagina = "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=1"
    
    mock_json_pagina = {
        "dados": [{"id": 12345}],
        "links": [{"rel": "next", "href": "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=2"}]
    }
...
@respx.mock
@patch("time.sleep")
def test_executar_pipeline_completo_sucesso(mock_sleep):
    """
    ...
    url_pag_2 = "https://dadosabertos.camara.leg.br/api/v2/deputados?idLegislatura=57&pagina=2"
    ...
    respx.get(url_pag_2).respond(
        status_code=200, 
        json={"dados": [{"id": 2}], "links": []}  # Sem próxima página
    )
    
    respx.get("https://dadosabertos.camara.leg.br/api/v2/deputados/1").respond(
        status_code=200, json={"dados": {"id": 1, "ultimoStatus": {"situacao": "Exercício"}}}
    )
    respx.get("https://dadosabertos.camara.leg.br/api/v2/deputados/2").respond(
        status_code=200, json={"dados": {"id": 2, "ultimoStatus": {"situacao": "Exercício"}}}
    )
    ...
    assert log_enviado["nome_rotina"] == "extrator_deputados"

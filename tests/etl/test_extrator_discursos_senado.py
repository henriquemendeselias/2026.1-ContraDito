...
def test_api_offset_e_headers(mock_get):
    ...
    mock_get.assert_called_once_with(url_esperada, headers={"Accept": "application/json"}, timeout=30.0, follow_redirects=True)
...
def test_api_retry_falhas(mock_get):
    ...
    mock_resp_429.raise_for_status.side_effect = httpx.HTTPStatusError("429 Rate Limit", request=MagicMock(), response=mock_resp_429)
...
def test_orquestracao_extracao_senador(mock_api, mock_html, mock_sleep):
    ...
    assert lote_enviado[0]["texto_bruto"] == "Olá. Este é um discurso simulado longo o suficiente para passar pela trava de segurança."

...
def test_gerar_id_proposicao_deve_ser_deterministico():
    ...
    assert id_1 == id_2, "O hash deve ser perfeitamente idempotente para a mesma entrada."
    
    # Valida se de fato é uma string UUID v5 bem formatada
    uuid_obj = uuid.UUID(id_1)
    assert uuid_obj.version == 5, "O hash gerado precisa ser obrigatoriamente um UUID versão 5."
...
def test_gerar_id_proposicao_lida_com_espacos_extras():
    ...
    assert id_limpo == id_sujo, "A função deve limpar os espaços (strip) antes de gerar o hash."
...
def test_obter_data_votacao_merito_encontra_whitelist():
    ...
    """
    tramitacoes_mock = [
        {"codTipoTramitacao": 232, "dataHora": "2023-06-20T10:00"}, # ALVO 2 (Veio primeiro na API - Decrescente)
        {"codTipoTramitacao": 100, "dataHora": "2021-01-01T10:00"}, # Irrelevante (Mais antigo)
        {"codTipoTramitacao": 231, "dataHora": "2023-05-15T15:30"}  # ALVO 1 (Primeira votação de mérito - Deve ser pego)
    ]
    
    data = obter_data_votacao_merito(tramitacoes_mock)
    assert data == "2023-05-15T15:30", "Deve capturar a data do primeiro evento validado da whitelist."
...
def test_obter_data_votacao_merito_ignora_irrelevantes():
    ...
    data = obter_data_votacao_merito(tramitacoes_mock)
    assert data is None, "Deve retornar None se a proposição nunca teve votação de mérito."
...
def test_transformar_proposicao_descarte_sem_votacao():
    ...
    assert resultado is None, "Proposições sem votação de mérito devem ser descartadas (None)."
...
def test_transformar_proposicao_descarte_data_antiga():
    ...
    resultado = transformar_proposicao(payload, tramitacoes)
    assert resultado is None, "Proposições com votação antes de 2023 devem ser descartadas (None)."
...
def test_transformar_proposicao_descarte_falso_positivo_cronologico():
    ...
    tramitacoes = [
        {"codTipoTramitacao": 231, "dataHora": "2023-08-10T14:00:00"}, # Votação recente (Falso positivo)
        {"codTipoTramitacao": 231, "dataHora": "2021-05-20T10:00:00"}  # Votação original (Texto-Base real)
    ]
    
    resultado = transformar_proposicao(payload, tramitacoes)
    assert resultado is None, "Deve ordenar, encontrar a de 2021 primeiro, e descartar o projeto."
...
def test_transformar_proposicao_sucesso_data_contract():
    ...
    tramitacoes = [
        {"codTipoTramitacao": 231, "dataHora": "2023-07-07T18:00:00", "id": "12345-tramitacao"}
    ]
    ...
    assert set(resultado.keys()) == chaves_exigidas, "O dicionário final deve obedecer estritamente ao Data Contract."

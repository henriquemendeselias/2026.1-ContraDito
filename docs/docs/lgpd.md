...
* **Isenção Algorítmica (Score de Coerência):** O cálculo do *Score de Coerência* não é uma avaliação subjetiva ou opinativa. Trata-se do resultado matemático e determinístico do cruzamento de dados executado pelo sistema. Para aprofundar, [entenda melhor no documento de Cálculo do Score](./calculo-score.md).
* **Rastreabilidade e Auditoria:** A rastreabilidade do sistema é garantida mediante a exibição do voto oficial registrado pelo parlamentar na proposição analisada (ex: PEC 45/2019), acompanhado da justificativa analítica elaborada pela Inteligência Artificial.
...
* **Minimização de Dados (PoC):** Visando garantir o acesso irrestrito e democrático à informação, a arquitetura atual do sistema **não exige criação de conta, login ou autenticação**. Não solicitamos, capturamos ou processamos Dados Pessoais Identificáveis (PII), como e-mail, nome ou CPF dos visitantes.

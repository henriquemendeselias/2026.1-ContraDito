# Links Úteis 

Abaixo estão os links oficiais para os principais recursos e ambientes do projeto **ContraDito**:

* **Repositório Oficial (GitHub):** [Acessar Repositório](https://github.com/unb-mds/2026.1-ContraDito)
* **Design e Protótipos (Figma):** [Acessar Board no Figma](https://www.figma.com/board/J6igyv5zX16YPhLaoKM3c4/ContraDito?node-id=0-1&t=Jq1ENqM8V4La6slk-0)
* **Dashboard de Produtividade:** [Acessar Dashboard](https://unb-mds.github.io/2026.1-ContraDito/productivity/)

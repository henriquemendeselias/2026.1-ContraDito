...
from app.modelos.schemas import PaginaPoliticosDB, PoliticoDetalhadoDB, PaginaDiscursosDB, DiscursoChunkDB, PaginaProposicoesDB, ProposicaoDB, PaginaVotosDB, VotoTimelineSchema, ComparacaoResponse, AfinidadesResponse, AfinidadeSchema, FidelidadeResponse, PolarizacaoResponse, CoesaoGeralResponse, DiscursoDB
...
def validar_casa(casa: str) -> str:
    ...
    if casa_clean not in VALID_CASES:
        raise HTTPException(status_code=400, detail=f"Casa inválida. Deve ser 'camara' ou 'senado'.")
    return casa_clean
...
def listar_politicos(
    ...
    estado: Optional[str] = Query(None, min_length=2, max_length=2, description="Filtro por Estado/UF"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao buscar políticos: {str(e)}")
...
def obter_politico_detalhado(
    ...
    try:
        ...
        res_politico = supabase.table(tabela_politico).select("*").eq("id", id_parlamentar).execute()
        ...
        return {
            "politico": politico_data,
            "resumo_votos": resumo_data
        }
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro interno ao buscar perfil detalhado: {str(e)}")
...
def listar_discursos(
    casa: str = Path(..., description="Casa legislativa ('camara' ou 'senado')"),
    politico_id: Optional[int] = Query(None, description="ID do político para filtrar discursos"),
    pagina: int = Query(1, ge=1, description="Número da página"),
    tamanho: int = Query(20, ge=1, le=100, description="Quantidade de itens por página"),
):
    casa_clean = validar_casa(casa)
...
def listar_discursos_politico(
    ...
    try:
        res_politico = supabase.table(tabela_politico).select("id").eq("id", id_parlamentar).execute()
        if not res_politico.data:
            raise HTTPException(status_code=404, detail="Político não encontrado")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao verificar existência do político: {str(e)}")
        
    tabela = f"{casa_clean}_discursos"
    try:
        query = supabase.table(tabela).select("*", count="exact").eq("politico_id", id_parlamentar)
        query = query.order("data_discurso", desc=True)
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar discursos do político: {str(e)}")
...
def obter_chunks_discurso(
    ...
    try:
        resultado = (
            ...
            .select("*")
            .eq("discurso_id", discurso_id)
            .execute()
        )
        return resultado.data
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao buscar chunks do discurso: {str(e)}")
...
def listar_proposicoes(
    ...
    tipo: Optional[str] = Query(None, description="Filtro por tipo de proposição (ex: PL, PEC)"),
    pagina: int = Query(1, ge=1, description="Número da página"),
    tamanho: int = Query(20, ge=1, le=100, description="Quantidade de itens por página"),
):
    casa_clean = validar_casa(casa)
...
def obter_proposicao_detalhada(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar proposição: {str(e)}")
...
def listar_votos(
    ...
    proposicao_id: Optional[str] = Query(None, description="Filtro por ID da proposição (código de texto)"),
    pagina: int = Query(1, ge=1, description="Número da página"),
    tamanho: int = Query(20, ge=1, le=100, description="Quantidade de itens por página"),
):
    casa_clean = validar_casa(casa)
...
def obter_timeline_votos(
    ...
    try:
        resultado = (
            ...
            .select(f"voto_oficial, proposicao_id, {tabela_proposicoes}(data_votacao, tipo, numero, ano, ementa)")
            .eq("politico_id", id_parlamentar)
            .execute()
        )
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter timeline de votos: {str(e)}")
...
def comparar_politicos(
    ...
    try:
        ...
        votos_1 = {
            v["proposicao_id"]: (v["voto_oficial"].strip().upper(), v.get(tabela_proposicoes, {}).get("ementa", ""))
            for v in res_votos_1.data
            if v.get("voto_oficial") and v["voto_oficial"].strip().upper() in ["SIM", "NÃO", "NAO"]
        }
        ...
        percentual = round((concordancias / total_comuns) * 100, 1) if total_comuns > 0 else 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao comparar políticos: {str(e)}")
...
def obter_afinidades_politico(
    ...
    try:
        res_politico = supabase.table(tabela_politicos).select("id").eq("id", id_parlamentar).execute()
        if not res_politico.data:
            raise HTTPException(status_code=404, detail="Político não encontrado")
        ...
        politicos_map = {p["id"]: p for p in res_todos_politicos.data if p["id"] != id_parlamentar}
        ...
        for p_id, votos_outro in votos_por_politico.items():
            ...
            concordancias = sum(1 for pid in comuns if votos_alvo[pid] == votos_outro[pid])
        ...
        comparacoes.sort(key=lambda x: (x["concordancia"], x["votos_comuns"]), reverse=True)
        gemeo = comparacoes[0]
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter afinidades políticas: {str(e)}")
...
def obter_fidelidade_partidaria(
    ...
    try:
        res_politico = supabase.table(tabela_politicos).select("id, partido").eq("id", id_parlamentar).execute()
        if not res_politico.data:
            raise HTTPException(status_code=404, detail="Político não encontrado")
        ...
        for v in votos_alvo:
            ...
            partido_votos = votos_partidarios.get(prop_id, {}).get(partido, {"SIM": 0, "NÃO": 0})
        ...
        taxa = round((votos_alinhados / total_votos_com_orientacao) * 100, 1) if total_votos_com_orientacao > 0 else 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter fidelidade partidária: {str(e)}")
...
def obter_polarizacao_proposicao(
    ...
    try:
        res_prop = supabase.table(tabela_proposicoes).select("id, proposicao_id").eq("id", id_proposicao).execute()
        ...
        res_votos = supabase.table(tabela_votos).select("voto_oficial").eq("proposicao_id", codigo_proposicao).execute()
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter polarização de proposição: {str(e)}")
...
def obter_coesao_partidos(
    ...
    try:
        res_votos = supabase.table(tabela_votos).select("proposicao_id, voto_oficial, partido_na_epoca").execute()
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter índice de coesão dos partidos: {str(e)}")
...
def obter_discurso_detalhado(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar discurso: {str(e)}")
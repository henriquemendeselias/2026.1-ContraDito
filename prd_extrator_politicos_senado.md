...
O sistema necessita de uma rotina automatizada e isolada para popular a tabela `politicos` no Supabase com os dados atualizados dos Senadores da República (Legislatura 57). 

Diferentemente da Câmara, a API de Dados Abertos do Senado retorna os dados primariamente em XML e utiliza uma lógica de histórico de mandatos/exercícios mais complexa. Além disso, existe um risco crítico de colisão de chaves primárias (IDs), já que um Senador e um Deputado podem compartilhar o mesmo número identificador nas respectivas APIs das Casas, o que corromperia o banco de dados unificado.
...
Para prevenir a colisão de IDs com a Câmara dos Deputados, o script aplicará matematicamente um "Offset" (soma de 1.000.000) ao ID original do Senado. O status do mandato será decodificado a partir da chave `Participacao`. 

Os dados serão limpos, transformados para o schema exato do banco e enviados ao Supabase por meio de um Bulk Upsert via SDK Oficial. O status da rotina será persistido na tabela `etl_logs`.
...
4. Como arquiteto de software, quero que todo ID original recebido da API do Senado seja acrescido de 1.000.000 antes de ser processado, para criar chaves primárias únicas e evitar sobrescrever um Deputado Federal no banco de dados (ex: Senador 150 vira ID 1000150).
...
10. Como administrador de banco de dados, quero que o script faça um único envio transacional (Bulk Upsert) da lista final validada para a tabela `politicos`, preservando a cota de tráfego do banco.
...
- **ID Offset:** `id_banco = id_original + 1000000`
- O dicionário persistido terá a seguinte estrutura rígida:
  - `id` (Aplicado o offset)
...
### Load (Supabase)
- A persistência utilizará a SDK do Supabase em Python: `supabase.table('politicos').upsert(lista_senadores).execute()`.
...
1. **Teste de ID e Offset:** Garantir que o valor ID recebido (ex: 500) resulta em `1000500` no dicionário formatado.
2. **Teste de Conversão de Status:** Validar os três ramos da árvore de decisão do Senado (Titular -> Ativo, Suplente em exercício -> Suplente, Afastado -> Inativo).
...
- Criação de tabela separada para Senadores (todos irão para a tabela global `politicos`).

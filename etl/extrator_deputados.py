...
def _fetch_com_retry(url: str, headers: Optional[dict] = None) -> Optional[dict]:
    ...
    for tentativa in range(3):
        try:
            ...
            if tentativa < 2:
                logger.warning(f"Erro ao acessar {url}. Retentando em {backoffs[tentativa]}s...")
...
def executar_extracao_pagina(url: str, supabase_client, ids_processados: set) -> Tuple[Optional[str], int]:
    """
    ...
    for item in dados_pagina:
        ...
        if id_deputado and id_deputado not in ids_processados:
            ...
            if detalhes:
                print(f"Deputado extraído com sucesso: {detalhes.get('nome_urna')} ({detalhes.get('partido')}-{detalhes.get('estado')})")
...
def executar_pipeline_completo(supabase_client) -> None:
    ...
    try:
        ...
        while next_url:
            next_url, linhas = executar_extracao_pagina(next_url, supabase_client, ids_processados)
            total_linhas += linhas
    ...
    supabase_client.table("etl_logs").insert({
        "nome_rotina": "extrator_deputados",

...
from etl.extrator_deputados import _fetch_com_retry
...
def extrair_senadores(url: str) -> List[dict]:
    ...
    for senador in lista_parlamentares:
        ...
        print(f"Senador extraído com sucesso: {identificacao.get('NomeParlamentar')} ({identificacao.get('SiglaPartidoParlamentar')}-{identificacao.get('UfParlamentar')})")
...
def executar_pipeline_senadores(supabase_client) -> None:
    ...
    supabase_client.table("etl_logs").insert({
        "nome_rotina": "extrator_senadores",

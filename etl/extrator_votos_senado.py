...
async def obter_sessao_votacao_senado(client: httpx.AsyncClient, id_senado: int) -> Optional[Tuple[int, str]]:
    """
    ...
    response = await client.get(url, headers={"Accept": "application/json"}, timeout=30.0)
...
async def extrair_votos_proposicao_senado(
    ...
    votos_transformados = [transformar_voto_senado(voto, proposicao_id) for voto in votos_validos]
    
    return votos_transformados, int(codigo_sessao) if codigo_sessao else None, data_sessao

async def executar_pipeline_votos_senado(supabase_client: Any, limite_amostral: Optional[int] = None) -> None:
    """
    ...
    try:
        ...
        resp_politicos = supabase_client.table("senado_politicos").select("id").execute()
        ...
        query_proposicoes = supabase_client.table("senado_proposicoes").select("proposicao_id, id_senado").is_("id_votacao_senado", "null")
        if limite_amostral:
        ...
        async with httpx.AsyncClient() as client:
            ...
            async def processar_proposicao_worker(prop: dict):
                async with semaphore:
                    ...
                    try:
                        votos, id_votacao, data_votacao = await extrair_votos_proposicao_senado(client, proposicao_id, id_senado, ids_validos_banco)
            ...
            for res in resultados:
                ...
                if votos:
                    ...
                    supabase_client.table("senado_votos").upsert(votos_deduplicados).execute()
                ...
                if id_votacao:
                    ...
                    supabase_client.table("senado_proposicoes").update(payload_update).eq("proposicao_id", proposicao_id).execute()

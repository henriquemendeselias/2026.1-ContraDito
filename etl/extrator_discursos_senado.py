...
from etl.transformadores_discursos_senado import normalizar_payload_senado, mapear_discurso_senado
...
def obter_html_discurso_senado(url_texto: str) -> str:
    ...
    try:
        response = _fetch_api_senado_com_retry(url_texto, headers={"User-Agent": "Mozilla/5.0"})
        return response.text
    except Exception as e:
        logger.error(f"Falha irrecuperável ao raspar HTML do discurso ({url_texto}): {e}")
        return "[ERRO DE REDE]"


def executar_extracao_senador(id_senador_banco: int, data_inicio: str, data_fim: str, supabase_client: Any) -> int:
    """
    ...
    """
    status_code, payload = obter_discursos_senador_api(id_senador_banco, data_inicio, data_fim)
    ...
    for discurso_raw in discursos_raw:
        ...
        discurso_mapeado = mapear_discurso_senado(id_senador_banco, discurso_raw, html_bruto)
...
def executar_pipeline_completo(supabase_client: Any, data_inicio: str, data_fim: str) -> None:
    """
    ...
    try:
        ...
        for senador in senadores:
            linhas = executar_extracao_senador(senador["id"], data_inicio, data_fim, supabase_client)
            total_linhas += linhas
...
if __name__ == "__main__":
    ...
    logger.info(f"Iniciando pipeline de extração do Senado ({data_inicio_arg} a {data_fim_arg})...")

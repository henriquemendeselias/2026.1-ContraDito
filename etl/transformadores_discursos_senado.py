...
def limpar_transcricao_senado(texto_bruto: str) -> str:
    ...
    texto_limpo = re.sub(r"^Texto integral\s*(não disponível!?)?\s*", "", texto_bruto, flags=re.IGNORECASE)
    ...
    texto_limpo = re.sub(r"\(([Rr]isos|[Pp]almas|[Vv]ozes|Ininteligível)\)", r"{\1}", texto_limpo)
    texto_limpo = re.sub(r"\s+", " ", texto_limpo)
    
    return texto_limpo.strip()


def mapear_discurso_senado(id_senador: int, discurso_raw: dict, html_bruto: str) -> dict:
    """
    ...
    if html_bruto == "[ERRO DE REDE]":
        ...
        texto_tag = (
            soup.find("div", id="textoIntegral") or
            soup.find("div", class_="texto-integral") or
            soup.find("div", id="textoDiscurso") or
            soup.find("div", class_="texto-pronunciamento") or
            soup.find("div", class_="texto")
        )
        ...
        if texto_tag:
            ...
            cabecalho = soup.find(lambda tag: tag.name in ["h2", "h3", "h4", "h5", "strong", "b", "span", "div"] and "Texto integral" in tag.get_text() and len(tag.get_text()) < 30)
            if cabecalho:
                paragrafos = [sibling.get_text(separator=" ", strip=True) for sibling in cabecalho.find_next_siblings("p")]
            ...
            if not texto_bruto:
                ...
                for i, p in enumerate(paragrafos):
                    ...
                    if re.match(r"^(?:O\s+SR\.?|A\s+SRA\.?|O\s+PRESIDENTE|A\s+PRESIDENTE)", txt, re.IGNORECASE):
                        texto_bruto = " ".join(tag.get_text(separator=" ", strip=True) for tag in paragrafos[i:])
                        break

        if texto_bruto:
            texto_bruto = limpar_transcricao_senado(texto_bruto)
            
        if not texto_bruto or len(texto_bruto) < 20 or "não disponível" in texto_bruto.lower():
            logger.error(f"Falha de Scraping. Texto não encontrado. URL: {discurso_raw.get('UrlTexto')}")
            texto_bruto = "[FALHA NO PARSER HTML]"

    return {
        "id": gerar_id_discurso_senado(id_senador, discurso_raw.get("CodigoPronunciamento", "")),

...
def contem_manobra_regimental_senado(descricao: str) -> bool:
    ...
    if not descricao:
        return False
    padrao = r"(?i)(requerimento|urgência|adiamento|destaque|questão\sde\sordem|preferência)"
...
def encontrar_sessao_merito_senado(sessoes: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """
...
def filtrar_votos_validos_senado(votos_brutos: List[Dict[str, Any]], ids_validos_banco: set) -> List[Dict[str, Any]]:
    """
...
def transformar_voto_senado(voto_bruto: Dict[str, Any], proposicao_id: str) -> Dict[str, Any]:
    """

...
from etl.transformadores_discursos import transformar_discurso
...
def extrair_pagina_discursos(url: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    for tentativa in range(max_tentativas):
        try:
            ...
            status = getattr(e.response, "status_code", None) if hasattr(e, "response") else "Rede/Timeout"
            ...
            if isinstance(status, str) or status >= 500 or status == 429:
                if tentativa < len(backoffs):
                    ...
                    logging.warning(f"Erro {status} na API da Câmara. Retentando em {espera}s... URL: {url}")
                    time.sleep(espera)
                    continue
                    
            logging.error(f"Falha irrecuperável ou limite de tentativas esgotado ({status}). URL: {url}")
            return [], None


def executar_extracao_deputado(id_deputado: int, data_inicio: str, data_fim: str, supabase: Any) -> int:
    """
...
def executar_pipeline_completo(supabase: Any, data_inicio: str, data_fim: str) -> None:
    ...
    for deputado in deputados:
        linhas = executar_extracao_deputado(deputado["id"], data_inicio, data_fim, supabase)
        total_linhas += linhas
        
    supabase.table("etl_logs").insert({
        "nome_rotina": "extrator_discursos",
        "status": "Concluído",

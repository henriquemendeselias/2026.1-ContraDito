...
from etl.transformadores_proposicoes_camara import transformar_proposicao, obter_data_votacao_merito, validar_corte_temporal
...
async def extrair_pagina_proposicoes(client: httpx.AsyncClient, url: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """
...
async def extrair_tramitacoes_proposicao(client: httpx.AsyncClient, id_proposicao: int, semaphore: asyncio.Semaphore) -> List[Dict[str, Any]]:
    """
...
async def extrair_detalhes_proposicao(client: httpx.AsyncClient, id_proposicao: int, semaphore: asyncio.Semaphore) -> Dict[str, Any]:
    """
...
async def processar_pagina_proposicoes(client: httpx.AsyncClient, url: str, semaphore: asyncio.Semaphore) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """
    ...
    for payload in dados_brutos:
        ...
        if id_proposicao:
            tarefas_tramitacao.append(extrair_tramitacoes_proposicao(client, id_proposicao, semaphore))
            payloads_ativos.append(payload)

    # Executa todas as buscas de tramitações concorrentemente
    resultados_tramitacoes = await asyncio.gather(*tarefas_tramitacao, return_exceptions=True)
    ...
    for i, payload in enumerate(payloads_ativos):
        ...
        if not isinstance(tramitacoes, Exception) and tramitacoes is not None:
            ...
            if validar_corte_temporal(data_votacao):
                detalhes = await extrair_detalhes_proposicao(client, payload.get("id"), semaphore)
                payload.update(detalhes) # Injeta a urlInteiroTeor e a ementa enriquecida no dict original
                
                proposicao_transformada = transformar_proposicao(payload, tramitacoes)
                if proposicao_transformada:
                    logger.info(f"Proposição aprovada no filtro: {proposicao_transformada['proposicao_id']} (Votação: {proposicao_transformada['data_votacao']})")
                    resultados_validos.append(proposicao_transformada)
        elif isinstance(tramitacoes, Exception):
            logger.error(f"Falha ao buscar tramitação para proposição {payload.get('id')}: {tramitacoes}")
...
async def executar_pipeline_completo(supabase_client: Any, data_inicio: str, data_fim: str) -> None:
    """
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            while atual_dt <= fim_dt:
                ...
                logger.info(f"Processando fatia temporal: {fatia_inicio} a {fatia_fim}...")
                ...
                while url_atual:
                    resultados_validos, next_url = await processar_pagina_proposicoes(client, url_atual, semaphore)
        ...
        if lote_completo:
            ...
            try:
                ...
                logger.error(f"Falha de Banco de Dados: Erro no momento de fazer o Bulk Upsert: {erro_db}")
                raise erro_db
        else:
            logger.info("O lote terminou vazio. Nenhuma proposição atendeu ao filtro rigoroso de Texto-Base no período.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Iniciando pipeline de extração de Proposições da Câmara ({data_inicio_arg} a {data_fim_arg})...")
    asyncio.run(executar_pipeline_completo(cliente_banco, data_inicio_arg, data_fim_arg))

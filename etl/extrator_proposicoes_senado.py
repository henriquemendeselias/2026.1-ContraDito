...
def gerar_hash_id_proposicao(sigla: str, numero: int, ano: int) -> Tuple[str, str]:
    ...
    """
    proposicao_id = f"{str(sigla).lower().strip()}_{str(numero).strip()}_{str(ano).strip()}"
    proposicao_uuid = str(uuid.uuid5(uuid.NAMESPACE_URL, proposicao_id))
    return proposicao_id, proposicao_uuid


def obter_data_primeira_votacao_valida(autuacoes: List[Dict[str, Any]]) -> Optional[str]:
    """
...
def transformar_proposicao_senado(payload: Dict[str, Any], url_documento: str, ementa: str) -> Optional[Dict[str, Any]]:
    """
...
async def extrair_detalhe_proposicao(client: httpx.AsyncClient, id_proposicao: int, semaphore: asyncio.Semaphore) -> Dict[str, Any]:
    """
    ...
    for i, res in enumerate(resultados_detalhe):
        ...
        if res:
            ...
            if detalhe_materia:
                ...
                proposicao_transformada = transformar_proposicao_senado(detalhe_materia, url_doc, ementa)
...
async def executar_pipeline_completo(supabase_client: Any, data_inicio: str, data_fim: str) -> None:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            while url_atual:
                linhas_salvas, url_atual = await processar_pagina_arrasto(client, url_atual, supabase_client, semaphore)

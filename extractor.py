        ...
        for lei in dados_leis.get("dados", []):
            ...
            for votacao in dados_votacoes.get("dados", []):
                ...
                for voto in dados_votos.get("dados", []):
                    ...
                    try:
                        supabase.table("provas_contradicao").insert(registro_voto).execute()
                        print(f"✅ Salvo no banco: ID {registro_voto['politico_id']} | Voto: {registro_voto['voto_oficial']}")
                    except Exception as erro_banco:
                        print(f"❌ Erro ao salvar o voto no ID {registro_voto['politico_id']}: {erro_banco}")

...
async def main():
    parser = argparse.ArgumentParser(description="Pipeline de Resumo e Vetorização de Proposições")
    parser.add_argument("--casa", type=str, choices=["camara", "senado", "ambas"], default="ambas", help="Qual casa processar.")
    parser.add_argument("--limite", type=int, default=None, help="Limite máximo de proposições a processar por Casa.")
    ...
    if args.casa in ["senado", "ambas"]:
        ...
        if not gemini_key:
            logging.error("GEMINI_API_KEY precisa estar definida para processar o Senado.")
            sys.exit(1)
